...
if(renderers.length<=1) fails.push('ROTATION: only '+renderers.length+' renderer string across '+bySeed.length+' seeds. Either the anchor pool has one member (check --fingerprint-explain for "has one measured member"), or the catalogue offers several and selection is pinning one. The second is a bug the pool widening did not fix.');
