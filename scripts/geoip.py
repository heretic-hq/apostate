...
The default transport uses :mod:`urllib.request`.  Tests and package adapters
may inject a callable accepting :class:`HTTPTransportRequest`, or a callable
with the equivalent ``(url, timeout, proxy)`` arguments.  The injected
transport owns the actual network operation and must honor ``timeout``.
...
# This is intentionally a conservative locale policy.  It is Apostate-owned
# mapping data, not a raw third-party corpus.  Missing countries remain
# unresolved instead of silently becoming en-US.
_COUNTRY_LOCALES = MappingProxyType(
    {
        "AR": "es-AR",
        "AT": "de-AT",
        "AU": "en-AU",
...
def parse_proxy_url(value: Union[str, ProxyConfig]) -> ProxyConfig:
    ...
    repr and ``redacted_url`` never contain them.  SOCKS endpoints are parsed
    for package-adapter use; the standard-library transport reports them as an
    explicit unsupported configuration because Python has no SOCKS client in
...
def _parse_provider_payload(payload: Mapping[str, Any]) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    locale = _COUNTRY_LOCALES.get(country) if country else None
...
def urllib_transport(request: HTTPTransportRequest) -> HTTPTransportResponse:
    ...
    if request.proxy is not None and not request.proxy.supports_urllib:
        raise GeoIPConfigurationError(
            f"The standard-library transport cannot use {request.proxy.scheme!r} proxies; install a SOCKS-capable adapter"
        )
    proxy_map = {}
